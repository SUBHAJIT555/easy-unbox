...
const PopularProducts = () => {
  ...
  if (!isLoading && !isError && products?.data?.length > 0) {
    // Already limited to 8 products from API
        ...
        {product_items.map((item) => (
          ...
          >
            <div className="tp-category-thumb-2">
              <div onClick={() => dispatch(handleProductModal(item))} style={{ cursor: 'pointer' }}>
            ...
            <div className="tp-category-content-2">
              ...
              <h3 className="tp-category-title-2">
                <a onClick={() => dispatch(handleProductModal(item))} style={{ cursor: 'pointer' }}>{item.title.substring(0, 15)}</a>
              </h3>
